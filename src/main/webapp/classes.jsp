...
<script type="text/javascript">
  ...
  $(function(){
	  $("#dg").datagrid({
		 idField:"id",
		 url:"ClassesqueryByPage",
		 queryParams:formToJson("searchform"),
				 columns:[[
					 {field:"",checkbox:true },
					 {field:"id",width:100,title:"班级编号",align:"center" },
					 {field:"name",width:100,title:"班级姓名",align:"center" }
		         ]],
					toolbar:[
						{
							text:"新增",
  ...
  function remove(){
    ...
    function update(){
        ...
        //表单提交 会获取表单的所有信息
	    $('#ff').form('submit',{
				url:'Classesupdate?id='+selectedRow.id, //selectedRow.empno 获取选中行的数据id
...
<div id="addDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form action="" method="post" id="zz" style="">
       <table>
			<tr><td>班级姓名：</td><td><input type="text" name="name" class="easyui-textbox" data-options="required:true"></td></tr>
       </table>
  </form>
</div>
...
<div id="editDialog" class="easyui-dialog" style="top:100px;padding:20px"   data-options="iconCls:'icon-save',resizable:true,modal:true,closed:true"  >
  <form  method="post" id="ff">
     <table>
			<tr><td>ID：</td><td><input type="text" name="id" class="easyui-textbox" data-options="required:true"></td></tr>
